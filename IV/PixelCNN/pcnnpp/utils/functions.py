...
def log_prob_hitmap_1d(x, l): #(img, output) (-1,1)
    """ log-likelihood for mixture of discretized logistics, assumes the data has been rescaled to [-1,1] interval """
    # Pytorch ordering
    x = x.permute(0, 2, 3, 1) #shape [64,28,28,1]
    l = l.permute(0, 2, 3, 1) #shape [64,28,28,9]
    xs = [int(y) for y in x.size()]
    ls = [int(y) for y in l.size()]
...
def get_hitmap_function(input_shape):
    if input_shape[0] == 3:
        print('use log_prob_hitmap')
        return log_prob_hitmap
    print('use log_prob_hitmap_1d')
