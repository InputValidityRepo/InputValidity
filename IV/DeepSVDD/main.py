...
from datasets.main import load_dataset_modified
...
def main(dataset_name, net_name, xp_path, data_path, load_config, load_model, objective, nu, device, seed,
    ...
    logger.info('Normal class: all normal')
    ...
    # Load data
    data_path = './data/'
    dataset = load_dataset_modified(dataset_name, data_path)
    ...
    if pretrain:
        ...
        # Pretrain model on dataset (via autoencoder), no labels are needed
    ...
    # Train model on dataset, no labels are needed
    deep_SVDD.train(dataset,
                    ...
                    device=device,
                    n_jobs_dataloader=n_jobs_dataloader)
    deep_SVDD.save_model('deepSVDD_ckpt_soft_boundary')
