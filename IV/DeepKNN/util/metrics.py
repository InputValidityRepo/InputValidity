...
def cal_metric(known, novel, gen_img_id, method, TIG, K):
    tp, fp, fpr_at_tpr95 = get_curve(known, novel, gen_img_id, method, TIG, K)
    results = dict()
...
def get_curve(known, novel, gen_img_id, method, TIG, K):
    tp, fp = dict(), dict()
    fpr_at_tpr95 = dict()
    ori_novel = novel.copy()
    ori_known = known.copy()
    known.sort()    #from smallest to biggest
    novel.sort()
    ...
    if method == 'row':
        ...
        threshold = known[round(0.05 * num_k)]  #thres only depends on valid data, 95%
    #Now compute validity of gen_imgs
    print('threshold', threshold)
    valid_count = 0
    with open('knnood_valid_percent_{0}_K={1}_Driving_PilotNet_dim10.txt'.format(TIG, K), 'w') as f:
        f.write('threshold at 95% = {0}'.format(threshold))
        f.write('\n')
        for idx, score_gen in enumerate(ori_novel):
            print('idx', idx, 'gen_img len', len(gen_img_id))
            if score_gen > threshold:
                print('this gen_img = {0} is valid'.format(gen_img_id[idx]))
                valid_count += 1
                f.write('this gen_img = {0} is valid'.format(gen_img_id[idx]))
                f.write('\n')
            else:
                print('this gen_img = {0} is invalid'.format(gen_img_id[idx]))
                f.write('this gen_img = {0} is invalid'.format(gen_img_id[idx]))
                f.write('\n')
        f.write('total generated imgs: {0}, valid counts: {1}, % Valid: {2}'.format(len(ori_novel), valid_count,
                                                                                    valid_count / len(ori_novel)))
        f.write('\n')
        f.close()
    ...
    fpr_at_tpr95 = np.sum(novel > threshold) / float(num_n) # score > threshold is valid
