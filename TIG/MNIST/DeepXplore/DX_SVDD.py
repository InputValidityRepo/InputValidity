...
if args.weight_svdd == 0:
    ...
    output_directory = './DX_SVDD' + str(args.target_model + 1)+'/'+(args.transformation)+'/' + str(knn_threshold) + '_' + str(args.weight_svdd)+'/'
...
while len(result_loop_index) < 110:
    ...
    layer_output = (loss1 + loss2 + loss3) + args.weight_nc * (loss1_neuron + loss2_neuron + loss3_neuron) + args.weight_svdd * (-1 * dist) #minimize distance from training
    ...
    iterate = K.function([input_tensor], [loss1, loss2, loss3, loss1_neuron, loss2_neuron, loss3_neuron, dist, grads])
